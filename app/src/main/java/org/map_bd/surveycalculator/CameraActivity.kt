...
import org.map_bd.surveycalculator.databinding.ActivityCameraBinding
import java.io.File
import java.text.DecimalFormat

class CameraActivity : AppCompatActivity(), PermissionCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mContext = this@CameraActivity

        // initialize the GeoTagImage class object with context and callback
        // use try/catch block to handle exceptions.
        geoTagImage = GeoTagImage(mContext as CameraActivity, this)
        ...
        binding.toggleAppearanceRandom.check(org.map_bd.surveycalculator.R.id.button_ext_png)
        binding.toggleAppearanceRandom.addOnButtonCheckedListener { group, checkedId, isChecked ->
            if (isChecked) {
                when (checkedId) {
                    org.map_bd.surveycalculator.R.id.button_ext_png -> {
                        geoTagImage.setImageExtension(PNG)
                    }

                    org.map_bd.surveycalculator.R.id.button_ext_jpeg -> {
                        geoTagImage.setImageExtension(JPEG)
                    }

                    org.map_bd.surveycalculator.R.id.button_ext_jpg -> {
        ...
        binding.toggleImageQuality.check(org.map_bd.surveycalculator.R.id.button_img_low)
        if (binding.toggleImageQuality.checkedButtonId == org.map_bd.surveycalculator.R.id.button_img_low){
            imageQuality = ImageQuality.LOW
        }
        binding.toggleImageQuality.addOnButtonCheckedListener { group, checkedId, isChecked ->
            if (isChecked) {
                when (checkedId) {
                    org.map_bd.surveycalculator.R.id.button_img_low -> {
                        imageQuality = ImageQuality.LOW
                    }

                    org.map_bd.surveycalculator.R.id.button_img_average -> {
                        imageQuality = ImageQuality.AVERAGE
                    }

                    org.map_bd.surveycalculator.R.id.button_img_high -> {
